...
#>
    ...
    # check whether $set-team-test is the vSwitch or the LBFO team name bound to a vSwitch
    # if there is an LBFO team with the name we simply use that
    $isLBFOTeam = Get-NetLbfoTeam -Name $set-team-test -ErrorAction SilentlyContinue
    if (-NOT $isLBFOTeam)
    {
        # check to see whether this is a vSwitch
        $isvSwitch = Get-VMSwitch $set-team-test -ErrorAction SilentlyContinue
        ...
        if ($isvSwitch)
        {
            ...
            if ($tmpTeam)
            {
                # we found the LBFO team attached to the vSwitch! Set that to $set-team-test. We'll rediscover the vSwitch later.
                $set-team-test = $tmpTeam.Name
            }
            else 
            {
                Throw "An LBFO team associated with $set-team-test could not be detected."    
            }
        }
        else 
        {
            Throw "Failed to find an LBFO team or vSwitch named $set-team-test`."
        }
    ...
    $configData = @{ NetLBFOTeam = Get-NetLbfoTeam -Name $set-team-test -ErrorAction SilentlyContinue }
