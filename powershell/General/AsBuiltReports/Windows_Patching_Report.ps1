...
$allhostscount=(get-content $location/patching_status/allhosts.txt).count
$failedcount=(get-content $location/patching_status/failed_unreachable.txt).count
$patchedcount=(get-content $location/patching_status/patched.txt).count
...
$document = Document $patching -Verbose {
    ...
    Header -Default {
        Paragraph -Style Header "Windows Patching Report - v2.0"
    }
    ...
    $revision3=[PSCustomObject]@{
        "Date" = "17-May-2023"
        "Version" = "1.2"
        "Description" = "Added Playbook time heading in 3.1"
        "Revised By" = "Deepak Gautam"
    }
    $revision4=[PSCustomObject]@{
        "Date" = "22-May-2023"
        "Version" = "1.3"
        "Description" = "Added Cover Page for Patching Report"
        "Revised By" = "Deepak Gautam"
    }
	...
	pagebreak
            ...
            BlankLine
            Paragraph 'Quick summary of Windows patching is as follows: '
            BlankLine
            $summary=[PSCustomObject]@{
                'All Servers' = $allhostscount
                'Patched Servers'=$patchedcount
                'Failed Servers'=$failedcount
                'Time Taken'= "$(get-timetaken -location $location) Hrs"
            }
            Table -Name 'Patching Summary' -InputObject $summary -Columns "All Servers","Patched Servers","Failed Servers","Time Taken"`
            -ColumnWidths 25,25,25,25 -Headers "All Servers","Patched Servers","Failed Servers","Time Taken" -Caption "Patching Summary"
            LineBreak
            BlankLine
			# Section 1
			Paragraph -Style Heading3 '1. Hosts Information'
			blankline
			Section -Style Heading4 '.1 All hosts'{
                ...
                $allhostco+=$patchco
                $allhostco+=$failco
                Table -Name 'Allhosts Information' -InputObject $allhostco -Columns "Name","Status" `
                -ColumnWidths 60,40 -Headers "Name","Status" -Caption "Allhosts Information" 
			}
