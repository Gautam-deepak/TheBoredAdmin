...
<# $event=get-winevent -computername umumdcads.upl.com -filterhashtable @{logname='Security';id=4740}
    $final=foreach($item in $event) {if($item.properties.value -contains 20014827) {$item | select-object -property `
    @('name','machineName','id','timeCreated','message','properties.value')}}
#>
