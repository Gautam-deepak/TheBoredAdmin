...
$results=$Computers | foreach-object -Parallel {
    ...
    If (test-connection -ComputerName $_ -Count 1 -Quiet){
        ...
        Copy-Item -Path "C:\Users\novaprime\Documents\PSWindowsUpdate" -Destination `
        "\\$_\c$\Windows\system32\WindowsPowerShell\v1.0\Modules" -Recurse -force -ErrorAction silentlycontinue;
        ...
        do {
            ...
            $updates=Get-WindowsUpdate -ComputerName $_ -kbarticleID KB4339284 -ErrorAction SilentlyContinue 
                
            if($? -eq $false){
                write-host "PSWinidowsUpdate module failed on $_" -ForegroundColor "red";
            ...
            if($updates.Count -eq 0){
                ...
                Install-WindowsUpdate -ComputerName $_ -kbarticleID KB4339284 -IgnoreReboot -Confirm:$false -Verbose | `
                ...
                if(Get-WURebootStatus -ComputerName $_  -Silent) {
                    ...
                    if($counter[$_] -eq "0"){
                        ...
                        if($? -eq $false){
                            ...
                            $KBinstalled=Invoke-Command -ComputerName 'server1' -ScriptBlock {
                            (Get-HotFix | Where-Object {$_.installedon -gt (get-date).AddDays(-1)} | `
                            Select-Object hotfixID -ExpandProperty hotfixID) -join ","} -ErrorAction SilentlyContinue
    ...
    try {
        $uptime=[System.Management.ManagementDateTimeconverter]::ToDateTime($(Get-WmiObject -ComputerName $_ `
        -Class Win32_OperatingSystem  | Select-Object -ExpandProperty LastBootUpTime))
    }
    catch {
        $uptime="failure"
    }
